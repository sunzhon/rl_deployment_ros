...
class ImuVn100 {
 public:
  ...
  bool Initialize(const std::string& deviceName,const int& baudRate);

  void Stream(bool async = true);

  void PublishData(const VnDeviceCompositeData& data);
  void updateImuData(const VnDeviceCompositeData& data);
  ...
  sensor_msgs::Imu getVn100ImuData();
 ...
 private:
  ...
  void FixImuRate();
  void LoadParameters(const std::string& deviceName,const int& baudRate);
