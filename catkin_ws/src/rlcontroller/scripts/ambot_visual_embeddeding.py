...
from a1_real import AmbotReal, resize2d
...
def get_encoder_script(logdir):
    ...
    model = getattr(modules, config_dict["runner"]["policy_class_name"])(
        num_actor_obs= ambot_real_env.num_obs,
        num_critic_obs= ambot_real_env.num_privileged_obs,
        num_actions= 12,
        obs_segments= ambot_real_env.obs_segments,
        privileged_obs_segments= ambot_real_env.privileged_obs_segments,
        **config_dict["policy"],
    )
...
def main(args):
    rospy.init_node("ambot_legged_gym_jetson")
